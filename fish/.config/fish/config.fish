...
#
set --global IFS \n\ \t
set --query --global __fish_added_user_paths
or set --global __fish_added_user_paths
...
if status --is-interactive
    ...
    if not contains -- "string" (builtin -n)
        set --global __is_launched_without_string 1
        ...
        if not set --query STY
            and not string match --quiet -- 'eterm*' $TERM
            and begin
                set --query KONSOLE_PROFILE_NAME # KDE's konsole
                or string match --quiet -- "*:*" $ITERM_SESSION_ID # Supporting versions of iTerm2 will include a colon here
                or string match --quiet -- "st-*" $TERM # suckless' st
            ...
            set --query fish_term24bit
            or set --global fish_term24bit 1
        end
    end
else
...
set --global __fish_config_dir ~/.config/fish

if set --query XDG_CONFIG_HOME
    set __fish_config_dir $XDG_CONFIG_HOME/fish
end

set --local userdatadir ~/.local/share
...
set --local __extra_completionsdir
set --local __extra_functionsdir
set --local __extra_confdir
...
if not set --query fish_function_path
...
if not set --query fish_complete_path
...
function __fish_reconstruct_path -d "Update PATH when fish_user_paths changes" --on-variable fish_user_paths
    set --local local_path $PATH

    for x in $__fish_added_user_paths
        set --local idx (contains --index -- $x $local_path)
        and set --erase local_path[$idx]
    end

    set --global __fish_added_user_paths
    if set --query fish_user_paths
        for x in $fish_user_paths[-1..1]
            if set --local idx (contains --index -- $x $local_path)
                set --erase local_path[$idx]
            else
                set --global __fish_added_user_paths $__fish_added_user_paths $x
            end
            set local_path $x $local_path
        end
    end

    set --export --global PATH $local_path
end
...
function __fish_on_interactive --on-event fish_prompt
    ...
    functions --erase __fish_on_interactive
end
...
if not set --query __fish_init_2_3_0
    if set --query fish_user_abbreviations
        set --local fab
    ...
    set --universal __fish_init_2_3_0
end

# macOS-ism: Emulate calling path_helper.
if command --search --quiet /usr/libexec/path_helper
    ...
    function __fish_macos_set_env -d "set an environment variable like path_helper does (macOS only)"
        ...
        set --export --global $argv[1] $result
    end
    ...
    functions --erase __fish_macos_set_env
end
...
if status --is-login
    ...
    #
    if test "$TERM" = linux
        if string match --quiet --ignore-case --regex '\.UTF' -- $LANG
            if command --search --quiet unicode_start
                unicode_start
            end
        end
    end
...
function __fish_expand_pid_args
    for arg in $argv
        if string match --quiet --regex '^%\d+$' -- $arg
